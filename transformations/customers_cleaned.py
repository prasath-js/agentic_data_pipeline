...
def main():
    ...
    try:
        ...
        # Ensure it has the expected columns for consistency, even if empty initially
        rejected_cols = customers_raw_df.columns.tolist() + ['rejection_reason']
        all_rejected_rows = pd.DataFrame(columns=rejected_cols)
        ...
        if not null_customer_id_rejected.empty:
            ...
            null_customer_id_rejected = null_customer_id_rejected.reindex(columns=rejected_cols)
        ...
        if 'join_date' in customers_df.columns:
            ...
            # NaT values will be sorted to the end by default, which is fine for 'last' position
            customers_df_sorted = customers_df.sort_values(by=['customer_id', 'join_date'], ascending=[True, False])
        else: # If join_date doesn't exist, just drop duplicates on customer_id
            customers_df_sorted = customers_df.sort_values(by=['customer_id'], ascending=[True]) # Consistent sort for deterministic keep

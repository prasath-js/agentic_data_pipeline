...
def main(con):
    ...
    if not rejected_df.empty:
        ...
        # Note: customer_id is DOUBLE, join_date is VARCHAR as per bronze.customers_raw
        ...
        # Prepare rejected rows for insertion, ensuring column order
        rejected_df_to_insert = rejected_df[[
            'customer_id', 'name', 'email', 'address', 'join_date', 
            '_source_file', '_ingest_ts', 'rejection_reason'
        ]]
        
        # Register the DataFrame with DuckDB for SQL insertion
        con.register('rejected_df_to_insert_temp', rejected_df_to_insert)
        con.execute("INSERT INTO rejected.rejected_rows SELECT * FROM rejected_df_to_insert_temp;")
        con.unregister('rejected_df_to_insert_temp') # Clean up registration
    ...
    for col in string_cols:
        # Apply strip only to non-null string values, preserving NaN
        customers_df[col] = customers_df[col].mask(customers_df[col].notna(), customers_df[col].astype(str).str.strip())
    ...
    if 'email' in customers_df.columns:
        # Convert to string first to handle potential non-string types or NaNs gracefully
...
# The main guard for standalone execution

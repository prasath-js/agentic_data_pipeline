...
# Constants will be injected by the execution environment
...
def main():
    ...
    # 1. Left-join transactions onto customers
    # 2. Identify rejected rows (transactions without a matching customer_id or with NULL customer_id)
    #    These will have NULL in customers.customer_id after the LEFT JOIN.
    #    We need to capture the original transaction details for rejected rows.
    rejected_query = """
    WITH joined_data AS (
        SELECT
            t.transaction_id,
    ...
    # 3. Process valid rows: aggregate, calculate, and assign spending tier
    gold_query = """
    WITH joined_and_filtered AS (
        SELECT
            c.customer_id,
            t.amount
    ...
    # 4. Write result to GOLD as customer_spending

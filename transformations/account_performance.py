...
def main():
    ...
    if not rejected_accounts_df.empty:
        ...
        # Check if rejected.rejected_rows exists
        ...
        if not table_exists:
            # Create table with schema matching rejected_accounts_df
            # Register a temporary view to infer schema from the DataFrame
            con.register("temp_rejected_accounts_df_view", rejected_accounts_df)
            con.execute("CREATE TABLE rejected.rejected_rows AS SELECT * FROM temp_rejected_accounts_df_view WHERE 1=0;")
            con.unregister("temp_rejected_accounts_df_view") # Unregister the temporary view
        
        # Prepare DataFrame for insertion into rejected.rejected_rows
        existing_rejected_cols_info = con.execute("PRAGMA table_info('rejected.rejected_rows');").df()
        existing_rejected_cols = existing_rejected_cols_info['column_name'].tolist()
        
        df_to_insert = pd.DataFrame(columns=existing_rejected_cols)
        for col in existing_rejected_cols:
            if col in rejected_accounts_df.columns:
                df_to_insert[col] = rejected_accounts_df[col]
            else:
                df_to_insert[col] = None # Add column with nulls if not present in rejected_accounts_df

        con.register("df_to_insert_rejected", df_to_insert)
        con.execute("INSERT INTO rejected.rejected_rows SELECT * FROM df_to_insert_rejected;")
        con.unregister("df_to_insert_rejected")
    ...
    # DuckDB might infer types differently, especially for empty results or specific values

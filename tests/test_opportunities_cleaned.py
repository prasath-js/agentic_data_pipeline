...
def test_rejected_rows_content_and_reason():
    ...
    try:
        ...
        assert not rejected_df.empty, "rejected.rejected_rows should not be empty if there are rejections"
...
def test_total_row_counts():
    ...
    try:
        ...
        # rejected_rows might not exist if there are no rejections, handle this gracefully
        rejected_count_query = con.execute(
            "SELECT count(*) FROM information_schema.tables WHERE table_schema = 'rejected' AND table_name = 'rejected_rows'"
        ).fetchone()[0]
        
        if rejected_count_query > 0:

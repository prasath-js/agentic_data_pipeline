...
def setup_bronze_data():
    ...
    # Ensure a clean slate by deleting the DB file if it exists
    if os.path.exists(DB_PATH):
        try:
            os.remove(DB_PATH)
        except OSError as e:
            print(f"Error removing existing DB file {DB_PATH}: {e}")
            # If removal fails, it might be locked. This could lead to test failures.

    # Open a dedicated connection for setup
    ...
    con_setup.close() # Close setup connection

    yield # Yield control to dependent fixtures/tests

    # Teardown: Open another connection for cleanup and remove the DB file
    if os.path.exists(DB_PATH):
        con_teardown = duckdb.connect(DB_PATH)
        con_teardown.execute("DROP TABLE IF EXISTS bronze.opportunities_raw")
        con_teardown.execute("DROP SCHEMA IF EXISTS silver CASCADE")
        con_teardown.execute("DROP SCHEMA IF EXISTS rejected CASCADE")
        con_teardown.close()
        try:
            os.remove(DB_PATH)
        except OSError as e:
            print(f"Error removing DB file {DB_PATH} during teardown: {e}")

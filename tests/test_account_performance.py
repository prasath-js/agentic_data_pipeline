...
def duckdb_connection():
    ...
    """
    # Ensure the DB file is clean before starting tests for this module
    if os.path.exists(DB_PATH):
        try:
            os.remove(DB_PATH)
        except PermissionError:
            print(f"Warning: Could not remove DB file {DB_PATH} before tests. It might be locked by another process. Attempting to proceed.")
        except Exception as e:
            print(f"Warning: An unexpected error occurred while removing DB file {DB_PATH} before tests: {e}")
...
def run_transformation(duckdb_connection):
    ...
    """
    con = duckdb_connection # Get the connection from the fixture
    ...
    # Run the main transformation function, passing the fixture's connection
    main(con)

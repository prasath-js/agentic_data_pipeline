...
def duckdb_connection():
    ...
    Ensures the database file is cleaned up before and after the module tests.
    """
    # Ensure the database file is removed before starting the module tests
    # This helps prevent PermissionError from previous runs.
    if os.path.exists(DB_PATH):
        try:
            os.remove(DB_PATH)
        except PermissionError:
            # If still locked, it's an external issue or a very sticky handle.
            # For CI/CD, this might require a more aggressive cleanup or a fresh environment.
            # For local dev, a restart might be needed.
            raise Exception(f"Failed to remove existing DB file {DB_PATH}. Please ensure no other process is using it.")
    ...
    if os.path.exists(DB_PATH):
        try:
            ...
            print(f"Warning: Could not remove DB file {DB_PATH} after tests. It might be locked by another process.")

@pytest.fixture(scope="module")
def run_transformation(duckdb_connection):
    """
    Sets up dummy silver tables and runs the main transformation function once per module.
    """
    ...
    # The main() function will create its own connection and close it.
    # This is important because the duckdb_connection fixture's connection
    # is still open and will be used by the tests.
    main()
    
    # Yield to allow tests to run, then cleanup (though cleanup is mostly handled by duckdb_connection teardown)
    yield

# All test functions now depend on `run_transformation` to ensure the pipeline has run
# and `duckdb_connection` to get a connection to the populated database.

def test_gold_table_exists_and_has_rows(duckdb_connection, run_transformation):
    con = duckdb_connection
...
def test_no_nulls_in_key_columns(duckdb_connection, run_transformation):
    con = duckdb_connection
...
def test_correct_data_types(duckdb_connection, run_transformation):
    con = duckdb_connection
...
def test_rejected_rows_table_exists(duckdb_connection, run_transformation):
    con = duckdb_connection
...
def test_rejected_rows_content(duckdb_connection, run_transformation):
    con = duckdb_connection
    ...
    gold_accounts = con.execute("SELECT account_name FROM gold.account_performance;").df()['account_name'].tolist()
    assert 'Null Account' not in gold_accounts, "Rejected account should not be in gold.account_performance."

def test_pipeline_value_calculation(duckdb_connection, run_transformation):
    con = duckdb_connection
...
def test_opportunity_count_calculation(duckdb_connection, run_transformation):
    con = duckdb_connection
...
def test_win_rate_calculation(duckdb_connection, run_transformation):
    con = duckdb_connection
...
def test_ranking_within_industry(duckdb_connection, run_transformation):
    con = duckdb_connection
    # Industry 'Tech':

...
def duckdb_connection():
    ...
    if os.path.exists(DB_PATH):
        try:
            ...
            # If removal fails (e.g., due to a lock), print a warning and try to proceed.
            # This might mean connecting to a non-clean DB, but it prevents test suite crash.
            print(f"WARNING: Could not remove existing DB file '{DB_PATH}' during setup: {e}")
            print("Attempting to connect to potentially existing DB. This might lead to inconsistent test results.")
...
def test_rejected_rows_table_exists(duckdb_connection):
    ...
    con = duckdb_connection
    df = con.execute("SELECT COUNT(*) FROM rejected.rejected_rows;").df() # Just checking if query runs
    assert df is not None, "rejected.rejected_rows table should exist."
...
def test_email_is_valid_logic(duckdb_connection):
    ...
    # Email with whitespace, should still be valid after strip

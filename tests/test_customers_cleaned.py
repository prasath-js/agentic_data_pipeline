...
def test_customers_cleaned_exists_and_has_rows():
    """Verify that silver.customers_cleaned table exists and contains data."""
    con = duckdb.connect(DB_PATH) # Open fresh connection
...
def test_no_null_customer_id_in_cleaned():
...
def test_email_is_valid_datatype():
...
def test_rejected_rows_reason_for_null_customer_id():
...
def test_duplicate_customer_id_resolution():

...
# This uses the DB_PATH constant injected by the environment,
# or defaults to a standard path if not set (e.g., for local dev/testing).
...
def setup_database():
    ...
    # Ensure the database file is removed before starting, to prevent stale locks
    if os.path.exists(DB_PATH):
        try:
            os.remove(DB_PATH)
            # Give the OS a moment to release the file handle, especially on Windows
            time.sleep(0.2) 
        except OSError as e:
            print(f"Warning: Could not remove DB file {DB_PATH} before test setup: {e}")
            # If removal fails due to lock, subsequent connect might also fail.
            # For robustness, we might want to wait or raise an error here.
    ...
    transactions_data = {
        ...
        'amount': [
            ...
            150.0, 250.0, # Feb: 400
            300.0, 100.0, # Mar: 400
            50.0,         # invalid-date (rejected)
            75.0,         # None (rejected)
            120.0, 180.0, # Apr: 300
            200.0,        # May: 200
            'invalid_amount' # May: invalid amount, dropped from aggregation
        ],
        'transaction_date': [
            ...
            '2023-05-01',
            '2023-05-10' # This date is valid, but amount is invalid.
        ]
    }
    ...
    if os.path.exists(DB_PATH):
        try:
            os.remove(DB_PATH)
            time.sleep(0.2) # Added sleep for robustness on Windows
        except OSError as e:
            print(f"Warning: Could not remove DB file {DB_PATH} after test teardown: {e}")
...
def test_rejected_rows_content(setup_database):
    con = setup_database
    rejected_df = con.execute("SELECT transaction_id, transaction_date, rejection_reason FROM rejected.rejected_rows ORDER BY transaction_id").df()
    assert len(rejected_df) == 2, "There should be 2 rejected rows for null year_month"
    
    # Transaction IDs 7 and 8 correspond to 'invalid-date' and None
    assert rejected_df.iloc[0]['transaction_id'] == 7
    assert rejected_df.iloc[0]['transaction_date'] == 'invalid-date'
    assert 'transaction_date could not be parsed' in rejected_df.iloc[0]['rejection_reason']

    assert rejected_df.iloc[1]['transaction_id'] == 8
    assert rejected_df.iloc[1]['transaction_date'] is None # Pandas reads SQL NULL as None
    assert 'transaction_date could not be parsed' in rejected_df.iloc[1]['rejection_reason']
...
def test_aggregation_and_mom_growth(setup_database):
    ...
    # Test May 2023 (only one valid transaction after 'invalid_amount' is dropped)
    may_data = df[df['year_month'] == '2023-05'].iloc[0]
    assert may_data['monthly_revenue'] == 200.0, "May monthly_revenue incorrect (should only count valid amount)"
    assert may_data['transaction_count'] == 1, "May transaction_count incorrect (should only count valid amount)"
...
def test_invalid_amount_handling(setup_database):
    con = setup_database
    # Check that the row with 'invalid_amount' was not included in the aggregation
    df = con.execute("SELECT year_month, monthly_revenue, transaction_count FROM gold.monthly_sales_summary WHERE year_month = '2023-05'").df()
    assert not df.empty, "May 2023 data should exist"
    assert df.iloc[0]['monthly_revenue'] == 200.0, "Monthly revenue for May should only include valid amounts"
    assert df.iloc[0]['transaction_count'] == 1, "Transaction count for May should only include valid amounts"
    
    # Also check that this row was NOT added to rejected.rejected_rows, as it's not a date parsing issue
    # The transaction_id for the invalid amount is 12
    rejected_df = con.execute("SELECT * FROM rejected.rejected_rows WHERE transaction_id = 12").df()
    assert rejected_df.empty, "Row with invalid amount should not be in rejected.rejected_rows for date parsing reason"
...
def duckdb_connection():
    ...
    # Ensure the database file is clean before tests run
...
def run_main_transformation(duckdb_connection):
    ...
    from __main__ import main # Import main from the transformation script
    main()
    del os.environ["DB_PATH"] # Clean up environment variable
    # No yield needed, just run the transformation
...
def test_correct_data_types(duckdb_connection):
    ...
    assert 'DECIMAL' in total_spent_dtype or 'DOUBLE' in total_spent_dtype, f"total_spent has incorrect type: {total_spent_dtype}"
    ...
    assert 'DECIMAL' in average_transaction_value_dtype or 'DOUBLE' in average_transaction_value_dtype, f"average_transaction_value has incorrect type: {average_transaction_value_dtype}"

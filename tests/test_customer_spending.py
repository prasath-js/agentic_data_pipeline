...
def duckdb_connection():
    ...
    con = None
    try:
        # Connect to DuckDB. If the file exists and is locked from a previous run,
        # duckdb.connect might still open it, or it might fail here.
        # The crucial part is to ensure proper cleanup in the 'finally' block.
        ...
        # Create dummy silver tables for testing
        customers_data = {
        ...
        if con:
            con.close()
        # Clean up the database file after tests, ensuring connection is closed first
...
def run_main_transformation(duckdb_connection):
    """Fixture to run the main transformation function once before tests."""
    # The main() function expects DB_PATH to be available globally or passed.
    # For pytest, we'll ensure it's set in the environment for the main() call.
    os.environ["DB_PATH"] = DB_PATH
    # Import main from the transformation script (assuming it's in the same file or accessible)
    # In a real scenario, you might import it directly if the transformation code is in a separate module.
    # For this setup, __main__ is appropriate.
    from __main__ import main 
    main()
    del os.environ["DB_PATH"] # Clean up environment variable
